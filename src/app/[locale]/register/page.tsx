  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ name, username, password, dairyName }),
      });
      ...
      <div className="w-full max-w-md space-y-8 bg-white p-8 rounded-2xl shadow-xl border border-slate-100 dark:bg-slate-800 dark:border-slate-700">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="dairyName" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
                {t('auth.dairyName')}
              </label>
              <input
                id="dairyName"
        ...
        <div className="text-center text-sm">
          <span className="text-slate-500 dark:text-slate-400">{t('auth.hasAccount')} </span>
          <Link href={`/${locale}/login`} className="font-semibold text-emerald-600 hover:text-emerald-500 dark:text-emerald-400">
            {t('auth.signIn')}
          </Link>
        </div>
